...
from src.handlers.write_character import edit_character
...
def update_character():
    ...
    character = Character(
        ...
        skills=[
            ...
            for skill_data in character_data.getlist('skills')
        ] or [],
        perks=[
            Perks(
                perk_id=perk_data.get('perk_id'),  # Add the perk_id if available
                name=perk_data['name'],
                rank=perk_data['rank'],
                description=perk_data['description']
            )
            for perk_data in character_data.getlist('perks')
        ] or []
    )
    
    if edit_character(character):
        response = {
            'status': 'success',
